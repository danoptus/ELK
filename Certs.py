...
if masters > 1:
    ...
    ip =  input("\nInforme o ip dos hosts masters: ")
     config_certs  = "/opt/elastic/elasticsearch-7.5.1/bin/elasticsearch-certutil cert --ca /opt/elastic/elasticsearch-7.5.1/config/certs/ca.p12 --ca-pass --ip {}  --out /opt/elastic/elasticsearch-7.5.1/config/certs/certs/master-{}.p12 --pass".format(ip,i)
     os.system(config_certs)
     
  
else:
 ip = input("\nInforme o ip do host master: ")
 config_certs = "/opt/elastic/elasticsearch-7.5.1/bin/elasticsearch-certutil cert --ca /opt/elastic/elasticsearch-7.5.1/config/certs/ca.p12 --ca-pass --ip {} --out /opt/elastic/elasticsearch-7.5.1/config/certs/certs/master-1.p12 --pass".format(ip)
 os.system(config_certs)
